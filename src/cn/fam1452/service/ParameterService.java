...
public class ParameterService extends Base {
	...
	public StringBuilder exportString(ParameteDataBo pdb) {
		String defaultTitle = "IONOSPHERIC DATA";//报表名称
		String year = pdb.getYear();
		String month = pdb.getMonth();
		String paraType = pdb.getParaType();
		...
		//sbtxt.append("station").append("\t") ;
		//sbtxt.append("data").append("\t") ;
		for (String tit : paraAry) {
			sbtxt.append(tit).append("\t") ;
		}
		sbtxt.append("\r\n") ;
		...
		for (String months : monthAry) {
			...
			for (String paraArys : paraAry) {
				...
				if (null != listData && listData.size() > 0) {
					for (int i = 0; i < listData.size(); i++) {
						ParameterMonthDateBo para = (ParameterMonthDateBo) listData.get(i);
						//sbtxt.append(pdb.getStationID()).append("\t") ;
						//sbtxt.append(DateUtil.convertDateToString(parat.getCreateDate(), "yyyyMMDDHH")).append("\t") ;
						for (int col2 = 0; col2 < 25; col2++) {
							if (col2 == 0) {
								sbtxt.append(para.getDays()).append("\t") ;
							} else {
								sbtxt.append(getParaValue(para, col2)).append("\t") ;
							}
						}
					}
					sbtxt.append("\r\n") ;
	 ...
	 * */
	public List top50ParameterDataList(Object obj, Parameter params, String tableName, ParameteDataBo paraQuery) {
			 ...
			 if(null!=prodata && null!=prodata.getId()){   		    	
	    		 ...
	    		 if(getProtectDateType(dataType,paraQuery)>=1){
		     	  ...
		     	  if(getProtectDateType(dataType,paraQuery)>=2){
	    			 ...
	    			 if(getProtectDateType(dataType,paraQuery)==2){
	    				 cnd= Cnd.where("createDate", ">=",dateQ1).and("createDate","<=",dateB1).or("createDate",">=",dateB2).and("createDate","<=",dateQ2).asc(paraQuery.getOrderBy()); 
			     	} 
	...
	/**
	 * 保护期公共数据查询  2013-02-21
	 * 暂不显示保护期内的数据，待对方测试出问题再说
	 * 若查询区间都在保护期内，只显示50条
	 * */
	public Condition getPublicProtectDataCnd(String dataType, ParameteDataBo paraQuery) {
		Condition cnd = null;
		if (StringUtil.checkNull(paraQuery.getOrderBy())) {
			paraQuery.setOrderBy("createDate");//默认排序方式：时间
		}
			...
			if (null != prodata && null != prodata.getId()) {
				...
				if (getProtectDateType(dataType, paraQuery) >= 1) {
					...
					if (getProtectDateType(dataType, paraQuery) >= 2) {
						...
						if (getProtectDateType(dataType, paraQuery) == 2) {
							cnd = Cnd.where("createDate", ">=", dateQ1).and("createDate", "<=", dateB1).or("createDate", ">=", dateB2).and("createDate", "<=", dateQ2).asc(
									paraQuery.getOrderBy());
						}
						if (getProtectDateType(dataType, paraQuery) == 3) {
							cnd = Cnd.where("createDate", ">=", dateQ1).and("createDate", "<=", dateB1).asc(paraQuery.getOrderBy());
	...
	public boolean isProtectDateOpen(String stationID, String dataTable, String startDate, String endDate) {
		...
		if (null != prodata && null != prodata.getId()) {
			...
			if (prodata.getPublicDate().getTime() > today.getTime()) {//当前的保护期未开放
				if (dateCompare(queryStart, prodata.getDataSDate(), prodata.getDataEDate()) || dateCompare(queryEnd, prodata.getDataSDate(), prodata.getDataEDate())
						|| dateCompare(prodata.getDataSDate(), queryStart, queryEnd) || dateCompare(prodata.getDataEDate(), queryStart, queryEnd)) {//查询区间与保护期有重叠
					return false;
				}
			}
	...
	/**
	 *电离参数生成TXT 文件
	 *return Java File Objet
	 **/
	public StringBuilder exportParaDataToTXT(List<Parameter> list) {
		String[] paraAry = Constant.paraAry;
		
		StringBuilder sbtxt = new StringBuilder() ;
//		sbtxt.append("Station   ").append(",\t") ;
//		sbtxt.append("Date").append(",\t") ;
		sbtxt.append("#").append("\t\t\t") ;
		for (String tit : paraAry) {
			sbtxt.append(tit).append(",\t") ;
		}
		...
		if (null != list && list.size() > 0) {
			for (int i = 0; i < list.size() ; i++){
				...
				sbtxt.append(StringUtil.empty2string(parat.getHlF(), "----")).append(",\t");
				sbtxt.append(StringUtil.empty2string(parat.getFoE(), "----")).append(",\t");
				sbtxt.append(StringUtil.empty2string(parat.getHlE(), "----")).append(",\t");
				sbtxt.append(StringUtil.empty2string(parat.getFmin(), "----")).append(",\t");
				sbtxt.append(StringUtil.empty2string(parat.getFoEs(), "----")).append(",\t");
				sbtxt.append(StringUtil.empty2string(parat.getFbEs(), "----")).append(",\t");
				sbtxt.append(StringUtil.empty2string(parat.getHlEs(), "----")).append(",\t");
				sbtxt.append(StringUtil.empty2string(parat.getEs(), "----")).append(",\t\t");
				sbtxt.append(StringUtil.empty2string(parat.getM3000F1(), "----")).append(",\t\t");
				sbtxt.append(StringUtil.empty2string(parat.getM3000F2(), "----")).append("\t");
				sbtxt.append("\r\n") ;
			}
		}else{
			sbtxt.append("数据为空");
...
sbtxt询保护期的情况
	...
	public String queryParaDataByQujian(ParameteDataBo pdb) {
		...
		String sql = " select " + pdb.getParaType() + ",createdate  from " + tableName + " where parameterID>0 and createDate >='" + dateQ1 + "' and createDate <'" + dateQ2
				+ "' and datepart(HH,createdate)='" + pdb.getMonth() + "' order by  createdate";
		return sql;
	}
